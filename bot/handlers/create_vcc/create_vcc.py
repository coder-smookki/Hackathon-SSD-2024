...
from bot.core.utils.utils import is_valid_email, parse_datetime
...
from bot.keyboards.universal import back_menu_keyboard, yes_no_keyboard
...
async def get_duration(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🙍 Введите максимальное количество людей в ВКС:",
        reply_markup=back_menu_keyboard,
    )
...
@create_vcc_router.callback_query(CiscoSettingsState.is_microphone_on, YesNo.filter())
async def start_get_cisco_settings(
    callback: CallbackQuery,
    callback_data: YesNo,
    state: FSMContext,
...
@create_vcc_router.callback_query(CiscoSettingsState.is_video_on, YesNo.filter())
async def start_get_cisco_settings(
    callback: CallbackQuery,
    callback_data: YesNo,
    state: FSMContext,
...
@create_vcc_router.callback_query(CreateVccState.participants, StopAddUser.filter())
...
@create_vcc_router.callback_query(CreateVccState.building, ChooseBuilding.filter())
...
@create_vcc_router.callback_query(CreateVccState.room, ChooseRoom.filter())
async def get_room(
    callback: CallbackQuery,
